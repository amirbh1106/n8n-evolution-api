	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (resource !== 'instance') {
				instanceName = this.getNodeParameter('instanceName', i) as string;
			}
			...
			if (resource === 'instance') {
				if (operation === 'createInstance') {
					...
					const name = this.getNodeParameter('newInstanceName', i) as string;
					const token = this.getNodeParameter('instanceToken', i) as string;
					const integration = this.getNodeParameter('connectionType', i) as string;
					...
					const instName = this.getNodeParameter('instanceNameParam', i) as string;
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'sendText') {
					...
					if (options.mentions) {
						...
						body.mentioned = (options.mentions as string).split(',').map((m) => m.trim());
					}
					...
					const lng = this.getNodeParameter('longitude', i) as number;
					...
					const reactionMsgId = this.getNodeParameter('reactionMessageId', i) as string;
					...
					const pollValues = (this.getNodeParameter('pollValues', i) as string)
						.split(',')
						.map((v) => v.trim());
					...
					endpoint = `/message/sendText/${instanceName}`;
					const text = this.getNodeParameter('text', i) as string;
					const quotedId = this.getNodeParameter('quotedMessageId', i) as string;
					...
					const msgId = this.getNodeParameter('messageId', i) as string;
					const editText = this.getNodeParameter('editText', i) as string;
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'findChats') {
					...
					const limit = this.getNodeParameter('messageLimit', i) as number;
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'createGroup') {
					...
					const groupJid = this.getNodeParameter('groupJid', i) as string;
					const pictureUrl = this.getNodeParameter('groupPictureUrl', i) as string;
					...
					const groupJid = this.getNodeParameter('groupJid', i) as string;
					const duration = this.getNodeParameter('ephemeralDuration', i) as number;
					body = { groupJid, expiration: duration };
				} else if (operation === 'updateSetting') {
					method = 'PUT';
					endpoint = `/group/updateSetting/${instanceName}`;
					const groupJid = this.getNodeParameter('groupJid', i) as string;
					const setting = this.getNodeParameter('groupSetting', i) as string;
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'fetchProfile') {
					...
					const name = this.getNodeParameter('profileName', i) as string;
					body = { name };
					...
					const pictureUrl = this.getNodeParameter('profilePictureUrl', i) as string;
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'setWebhook') {
					...
					const byEvents = this.getNodeParameter('webhookByEvents', i) as boolean;
					const base64 = this.getNodeParameter('webhookBase64', i) as boolean;
					body = {
						webhook: {
							url: webhookUrl,
							enabled: true,
			...
			// ══════════════════════════════════════════════
				...
				if (operation === 'findLabels') {
					...
					const chatJid = this.getNodeParameter('labelChatJid', i) as string;
